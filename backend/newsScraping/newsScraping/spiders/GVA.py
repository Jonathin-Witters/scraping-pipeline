...
class GVASpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        filename = f"gva-{page}.html"
        Path(filename).write_bytes(response.body)
        self.log(f"Saved file {filename}")

    def parse_article(self, response):
        yield {
            "title": response.css('h1[class="Heading_heading__okScq Heading_heading--2xl__7ZxPA story-headline_storyHeadlineHeading__3WzwH"]::text').get(),
